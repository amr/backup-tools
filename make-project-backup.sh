...
if [ "$PROJECT_MYSQL_DATABASES" != "none" ]; then
	DATABASES=$(env PROJECT_MYSQL_DATABASES="$PROJECT_MYSQL_DATABASES" MYSQL_USER="$MYSQL_USER" MYSQL_PASSWORD="$MYSQL_PASSWORD" MYSQL_HOST="$MYSQL_HOST" PROJECT_NAME="$PROJECT_NAME" TMP_DIRECTORY="$TMP_DIRECTORY" toolbox/mysql.sh)
	if [ $? == 0 ]; then 
